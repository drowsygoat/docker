...
# Check if the provided version matches "stable", "preview", or "daily"
if [ "$RSTUDIO_VERSION" = "stable" ] || [ "$RSTUDIO_VERSION" = "preview" ] || [ "$RSTUDIO_VERSION" = "daily" ]; then
    wget "https://rstudio.org/download/latest/${RSTUDIO_VERSION}/server/focal/rstudio-server-latest-${ARCH}.deb" -O "$DOWNLOAD_FILE" || {
        echo "Failed to download RStudio Server. Please check the URL or the network connection."
        exit 1
    }
else
    # Use the specific version provided
    RSTUDIO_VERSION_DASH=${RSTUDIO_VERSION//+/-}
    wget "https://download2.rstudio.org/server/focal/amd64/rstudio-server-${RSTUDIO_VERSION_DASH}.deb" -O "$DOWNLOAD_FILE" || {
        echo "Failed to download RStudio Server version ${RSTUDIO_VERSION}. Please check the URL or the version number."
        exit 1
    }
fi

sudo gdebi -n "$DOWNLOAD_FILE"
rm "$DOWNLOAD_FILE"
